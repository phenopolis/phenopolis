...
// highlight pubmed
function highlightPubmed (string, genes, terms){
    var gene_regex_string = '(\\b' + genes.join('\\b|\\b') + '\\b)';
    var gene_regex = new RegExp( gene_regex_string, 'gi' );
    var return_string = string.replace( gene_regex, '<span class="gene-highlight">$1</span>' );
    var term_regex_string = '(\\b' + terms.join('\\b|\\b') + '\\b)';
    var term_regex = new RegExp( term_regex_string, 'gi' );
    return return_string.replace( term_regex, '<span class="term-highlight">$1</span>' );
}
// check if an element is in viewport
function isElementInViewport (el) {
    if (!$(el).closest('.tab-pane').hasClass('active')){
        return false;
    }
