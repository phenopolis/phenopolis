...
def pubmedbatch_main():
    ...
    # It allows a user to create a "folder" to hold results in the mongodb. Post request will handle the 'folder' creation
    '''
    db.results = {
            'user_id':'Jing',
            'folder':{
...
def pubmedbatch(folder):
    ...
    if request.method != 'POST':
        ...
        user_folders = db.results.find_one({ 'user_id': user},{'_id': 0})['folder']
        # get AND an OR field
        ...
        if folder not in user_folders: return "Error: " + folder + " does not exist!"
    ...
    if bad: print 'weird, progress id conflict!'
    ...
    reg = re.compile( '\\b' + '\\b|\\b'.join(OR) + '\\b', re.IGNORECASE) 
    AND = AND_term.split()
    AND.sort()
    smashed_OR = ' OR '.join(['"' + t + '"' + '[Title/Abstract]' for t in OR])
    smashed_AND = ' AND '.join(['"' + t + '"' + '[Title/Abstract]' for t in AND])
    smashed_term = ' AND (' + smashed_OR + ')'
    if smashed_AND: smashed_term += ' AND ' + smashed_AND
    ###########################################################
    ...
    for row in csvreader:
        ...
        if gene_name in mask_genes:
            ...
            if not (verbose or genes[gene_name]['total_score']): continue
            # known genes?
            ...
            row[0:0] = [0]
            ha = {}
            for col in range(len(header)): ha[header[col]] = row[col]
            ha['pred_score'] = get_pred_score(ha)
            output.append(ha) 
    ...
    db.results.update({'user_id': user}, { "$set": {the_file: final_result} })
