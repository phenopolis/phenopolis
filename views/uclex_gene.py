...
def hide_hpo_for_demo(data):
    ...
    for h,v in data.iteritems():
        result[h] = {
                ...
                'expect_freq':v['expect_freq'],
                'data':{}
                }
        for p,val in v['data'].iteritems():
            #hide pid and hpo
            if p not in patients:
                patients[p] = obs + str(ind)
                ind += 1
            result[h]['data'][patients[p]] = {'hpo':['hidden'], 'var':[]}
            for var in val['var']:

                result[h]['data'][patients[p]]['var'].append({
                    'variant_id':'hidden',
                    'exac':var['exac'],
                    'cadd':var['cadd']
                    })
